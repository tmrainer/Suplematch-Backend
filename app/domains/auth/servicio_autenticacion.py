...
from app.domains.auth.esquemas import (
    LoginInput,
    MessageOut,
    PasswordChangeInput,
    PasswordForgotInput,
...
from app.domains.users.servicio_usuarios import to_user_out
...
class AuthService:
    ...
    def register(self, data: RegisterInput) -> TokenResponse:
        display_name = data.display_name or f"{data.first_name} {data.last_name}".strip()
        try:
            ...
            self.repo.update_personal_info(
                user,
                {
            ...
            self.repo.update_profile(
                user,
                {
                    ...
                    "health_goals": {
                        "registration_age_years": data.age,
                        "registration_weight_value": data.weight_value,
                        "registration_weight_unit": data.weight_unit,
                        "registration_weight_kg": data.weight_kg,
                        "registration_height_value": data.height_value,
                        "registration_height_unit": data.height_unit,
                        "registration_height_cm": data.height_cm,
                    },
                },
            )
