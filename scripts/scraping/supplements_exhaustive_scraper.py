...
DEFAULT_OUTPUT = ROOT_DIR / "data/raw/csv/supplements_exhaustive_clean.csv"
...
DEFAULT_COMPONENT_MASTER = ROOT_DIR / "data/training/modelo2/Component_Master_Clean.csv"
...
REGISTRO_RE = re.compile(
    r"\b(?:DE|EE|PNE|PN|N|I|E|DM|DB|Q|A)[-\s]?\d{3,9}(?:[A-Z0-9-]{0,12})\b",
    re.IGNORECASE,
)
...
def extract_registro(text: str) -> str:
    matches = [m.group(0).upper().replace(" ", "-") for m in REGISTRO_RE.finditer(text or "")]
    if not matches:
        return ""
    return matches[0]
...
class RegistryMatcher:
    ...
    def enrich(self, row: ProductRow, infer_missing_rs: bool) -> ProductRow:
        ...
        if not rs_key and infer_missing_rs:
            inferred = self.infer_rs(row.commercial_name)
            if inferred:
                row.registro_sanitario = inferred
                row.registro_sanitario_source = "digemid_name_inference"
    ...
    def infer_rs(self, product_name: str) -> str:
        query = normalize_name(product_name)
        if not query:
            return ""
        ...
        return self.digemid_by_rs.get(best_rs, {}).get("item", "") if best_score >= 0.92 else ""


class BaseScraper:
    def __init__(self, client: httpx.Client, limit: int, delay: float):
        self.client = client
        self.limit = limit
        self.delay = delay
...
class InretailAlgoliaScraper(BaseScraper):
    ...
    def __init__(self, client: httpx.Client, limit: int, delay: float, pharmacy: str, base_url: str):
        super().__init__(client, limit, delay)
    ...
    def _row_from_hit(self, hit: dict[str, Any]) -> ProductRow | None:
        ...
        return ProductRow(
            ...
            registro_sanitario_source="page_or_detail_regex" if registro else "",
        )
...
class VtexScraper(BaseScraper):
    ...
    def _row_from_product(self, product: dict[str, Any]) -> ProductRow | None:
        ...
        return ProductRow(
            ...
            registro_sanitario_source="vtex_spec_or_regex" if registro else "",
        )
...
class WooStoreScraper(BaseScraper):
    ...
    def _row_from_product(self, product: dict[str, Any]) -> ProductRow | None:
        ...
        return ProductRow(
            ...
            scraped_at=now_iso(),
            component_text=text,
            registro_sanitario_source="description_regex" if registro else "",
        )
...
class HtmlSearchScraper(BaseScraper):
    ...
    def _row_from_page(self, url: str) -> ProductRow | None:
        ...
        return ProductRow(
            ...
            registro_sanitario_source="product_page_regex" if registro else "",
        )
        ...
        for block in product_blocks:
            ...
            rows.append(
                ProductRow(
                    ...
                    registro_sanitario_source="search_tile_regex" if extract_registro(page_text) else "",
                )
            )
        return rows
...
def build_scrapers(client: httpx.Client, selected: set[str], limit: int, delay: float) -> list[BaseScraper]:
    definitions: dict[str, BaseScraper] = {
        "inkafarma": InretailAlgoliaScraper(client, limit, delay, "Inkafarma", "https://inkafarma.pe"),
        "mifarma": InretailAlgoliaScraper(client, limit, delay, "Mifarma", "https://www.mifarma.com.pe"),
        "boticasperu": BoticasPeruScraper(client, limit, delay),
        "farmaciauniversal": VtexScraper(client, limit, delay, "Farmacia Universal", "https://www.farmaciauniversal.com"),
        "hogarysalud": WooStoreScraper(client, limit, delay, "Hogar y Salud", "https://www.hogarysalud.com.pe"),
        "boticasysalud": HtmlSearchScraper(client, limit, delay, "Boticas y Salud", "https://www.boticasysalud.com"),
    }
    if not selected:
...
def main() -> int:
    ...
    with httpx.Client(headers=headers, follow_redirects=True) as client:
        for scraper in build_scrapers(client, selected, args.limit_per_pharmacy, args.delay):
