...
from app.domains.catalog.seguridad_productos import catalog_verification_status, infer_restriction_flags, verified_restriction_flags
    ...
    def _link_product_component(
        ...
        stale_links = db.scalars(
            select(CommercialProductComponent).where(
                ...
                CommercialProductComponent.source == "approved_catalog_csv",
            )
        ).all()
        for stale_link in stale_links:
            db.delete(stale_link)

        if ingredient and "CREATIN" in ingredient.upper() and "PANCREATIN" not in ingredient.upper():
            stale_creatine_links = db.scalars(
                select(CommercialProductComponent).where(
                    CommercialProductComponent.product_id == product.id,
                    CommercialProductComponent.ingredient.ilike("%CREATIN%"),
                    ~CommercialProductComponent.ingredient.ilike("%PANCREATIN%"),
                    ~CommercialProductComponent.ingredient.ilike("%CREATINOL%"),
                    CommercialProductComponent.component_id != component.id,
                    CommercialProductComponent.source == "approved_catalog_csv",
                )
            ).all()
            for stale_link in stale_creatine_links:
                db.delete(stale_link)
